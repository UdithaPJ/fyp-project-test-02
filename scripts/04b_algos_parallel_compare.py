...
def _run_algo(algo, graph_path, out_dir):
    with threadpool_limits(limits=1):
        ...
        if algo == "pagerank":
            pr = pagerank_csr(A)
            res = {"algo": algo, "seconds": time.time()-t0, "top20": np.argsort(pr)[-20:][::-1].tolist()}
        elif algo == "rwr":
            r = rwr_csr(A, seed=0)
            res = {"algo": algo, "seconds": time.time()-t0, "top20": np.argsort(r)[-20:][::-1].tolist()}
        elif algo == "hits":
            a, h = hits_csr(A)
            res = {
                "algo": algo,
...
def run_compare(graph_path, mode="single"):
    ...
    if mode == "single":
        ...
        with ProcessPoolExecutor(max_workers=len(algos)) as ex:
            results = list(ex.map(lambda a: _run_algo(a, graph_path, out_dir), algos))
    total = time.time() - t0

    summary = {"mode": mode, "graph_path": os.path.abspath(graph_path), "total_seconds": total, "results": results}
